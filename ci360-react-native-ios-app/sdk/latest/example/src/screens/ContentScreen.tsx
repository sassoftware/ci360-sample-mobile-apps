...
const ContentScreen: React.FC<ContentScreenProps> = () => {
  ...
  const [inlineViewNotVisible, setInlineViewNotVisible] = useState<boolean>(false);
  const inline_spotId = 'snzrle_inlineAd';
  const interstitial_spotId = 'snzrle_interstitialAd';
  ...
  useEffect(() => {
    if (Platform.OS === 'ios' && iOSMessagingEvent) {
      iOSMessagingEvent.addListener(Constants.AD_LOADED, (obj: { [key: string]: string }) => {
      ...
      iOSMessagingEvent.addListener(Constants.AD_DEFAULT_LOADED, (obj: { [key: string]: string }) => {
        const event = obj['eventType'];
        if (event === Constants.TYPE_INTERSTITIAL_AD) {
      ...
      DeviceEventEmitter.addListener(Constants.AD_LOADED, (obj: { [key: string]: string }) => {
        const event = obj['eventType'];
        if (event === Constants.TYPE_INTERSTITIAL_AD) {
          Toast.show('Interstitial Ad view is loaded', Toast.SHORT);
        } else if (event === Constants.TYPE_INLINE_AD) {
      ...
      DeviceEventEmitter.addListener(Constants.AD_DEFAULT_LOADED, (obj: { [key: string]: string }) => {
        const event = obj['eventType'];
        if (event === Constants.TYPE_INTERSTITIAL_AD) {
          Toast.show('Interstitial Ad default view is loaded', Toast.SHORT);
        } else if (event === Constants.TYPE_INLINE_AD) {
      ...
      <View style={styles.container}>
        ...
        <View style={styles.inlineViewContainer}>
          ...
          <InlineAdView
            ...
            style={inlineViewNotVisible ? styles.inlineViewWithoutContent : styles.inlineViewWithContent}
          />
        </View>
        ...
        <View style={styles.interstitialViewContainer}>
          ...
          />
          {showInterstitial && (
            <InterstitialAdView spotId={interstitial_spotId} viewId="interstitialViewId1" />
          )}
        </View>
      </View>
    </ScrollView>
