...
import { navigate, RootTabParameterList, navigationRef } from './RootNavigation';
...
const HomeStackNavigator = () => (
  <HomeStack.Navigator screenOptions={{
    ...
    <HomeStack.Screen name="HomeStack" component={HomeScreen} options={{ title: '' }} />
    <HomeStack.Screen name="Notifications" component={NotificationsScreen} options={{ tabBarStyle: { backgroundColor: '#d3d3d3' }, tabBarInactiveTintColor: 'grey' }} />
...
async function checkLocationPermission() {
  if (Platform.OS === 'ios') {
    ...
    if (status1 !== RESULTS.GRANTED && status2 === RESULTS.GRANTED && status3 === RESULTS.GRANTED) {
      ...
      Please set location permission to always to enable geofence', Toast.SHORT);
    }
  }
}
    ...
    <UserProvider>
      <NavigationContainer linking={linking} ref={navigationRef} 
        onStateChange={() => {
          ...
          if (currentRoute) {
            newPage(currentRoute.name); console.log('new page:', currentRoute.name);
          }
        }}
        onReady={() => {
          const currentRoute = navigationRef.getCurrentRoute();
