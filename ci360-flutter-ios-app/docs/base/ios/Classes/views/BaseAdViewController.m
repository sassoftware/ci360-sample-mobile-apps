...
//    [NSException raise:@"initNotImplemented" format:@"Subclass must implement a valid iniy"];
//}
...
-(instancetype)initWithMessenger:(NSObject<FlutterBinaryMessenger>*)messenger channelId:(NSString*)channelId ad:(SASIA_AbstractAd*)ad spotId:(NSString*)spotID {
    ...
    _methodChannel = [FlutterMethodChannel methodChannelWithName:channelId binaryMessenger:messenger];
    _view = ad;
    _spotID = [spotID mutableCopy];
...
-(void)onMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    if([[call method] isEqualToString:@"showAd"] && [_view class] == [SASCollectorInterstitialAd class]) {
        SASCollectorInterstitialAd *interstitialAd = (SASCollectorInterstitialAd*)_view;
        [interstitialAd load];
