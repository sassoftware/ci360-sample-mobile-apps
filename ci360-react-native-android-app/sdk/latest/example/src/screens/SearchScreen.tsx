...
//# File Name: SearchScreen.tsx                                                                                   #
//# File Description: Search screen that demonstrates content and spot retrieval based on search-driven interactions. #
...
const SearchScreen: React.FC<SearchScreenProps> = () => {
  ...
  const adSpots: { [key: string]: { jsonSpotId: string; htmlSpotId: string } } =
    {
      cat: { jsonSpotId: 'gp_CatJsonSpot', htmlSpotId: 'gp_CatHtmlSpot' },
      dog: { jsonSpotId: 'gp_DogJsonSpot', htmlSpotId: 'gp_DogHtmlSpot' },
      default: { jsonSpotId: 'gp_JsonSpot', htmlSpotId: 'gp_HtmlSpot' }, // Fallback
    };
  ...
  return (
    <View style={styles.container}>
      ...
      {jsonSpotData && (
        <JsonCreativeView
          spotId={
            adSpots[keyword.toLowerCase()]?.jsonSpotId ||
            adSpots['default'].jsonSpotId
          }
        ...
        <WebView
          ...
          onMessage={(e) =>
            handleWebviewClicked(
              e,
              adSpots[keyword.toLowerCase()]?.htmlSpotId ||
                adSpots['default'].htmlSpotId
            )
          }
        />
      ) : (
        <Text style={localStyles.noAdsText}>
          No ads to display. Enter a keyword.
        </Text>
      )}
    </View>
  );
